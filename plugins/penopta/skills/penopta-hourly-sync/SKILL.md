...
<!-- Generated by npm run plugins:publish — do not edit. Source: sync-skill + SYNC_SKILL_VERSION=2 -->
...
Set `provider` to `"chatgpt"`. Pass that same `provider` to every Penopta project-catalog MCP call in this run.
...
```json
{
  ...
  "agent": {
    "name": "chatgpt",
    "model": "<exact-model-id-if-known, e.g. gpt-5; otherwise \"unknown\">",
...
2. **Delivery or auth failed while Penopta tools are callable**
   ...
   - Call `penopta_diagnose({ agent: "chatgpt" })` **once** (do not retry in a loop).
